...
if TYPE_CHECKING:
    from claudeassistant.screens.chat import ChatScreen
...
from claudeassistant.sessions import (
    find_session_by_prefix,
    get_context_from_session,
    get_plan_path_for_session,
    get_recent_sessions,
)
from claudeassistant.commands import BARE_WORDS, handle_command
...
from claudeassistant.history import append_to_history
...
from claudeassistant.profiling import profile
from claudeassistant.tasks import create_safe_task
from claudeassistant.sampling import start_sampler
...
class ChatApp(App):
    ...
    def _track_edited_file(self, tool: ToolUse, file_path: Path) -> None:
        ...
        from claudeassistant.formatting import make_relative
    ...
    def LOCAL_COMMANDS(self) -> list[str]:
        from claudeassistant.commands import get_autocomplete_commands
    ...
    def _make_options(
        ...
        context_file = files("claudeassistant").joinpath("context.md")
    ...
    async def on_mount(self) -> None:
        ...
        if self._remote_port:
            from claudeassistant.remote import start_server
    ...
    def _show_system_info(
        ...
        from claudeassistant.filters import should_filter_message
    ...
    def _position_right_sidebar(self) -> None:
        ...
        has_content = agent_count > 1 or self.todo_panel.todos
    ...
    def run_shell_command(
        ...
        from claudeassistant.shell_runner import run_in_pty_cancellable
        from claudeassistant.widgets import PendingShellWidget, ShellOutputWidget
    ...
    async def _create_new_agent(
        ...
        if resume_id:
            ...
            self.notify(f"Agent '{name}' ready")
    ...
    def on_complete(self, agent: Agent, result: ResultMessage | None) -> None:
        ...
        if result and result.is_error:
            ...
            chat_view = self._chat_views.get(agent.id)
            if chat_view:
                from claudeassistant.widgets.content.message import SystemInfo
